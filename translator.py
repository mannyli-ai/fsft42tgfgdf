"""用 OpenAI API 把 Techmeme 標題和節錄翻譯成台灣繁體中文。

為了省錢省時間，一次 API 呼叫批次翻譯多則新聞，
要求模型回傳 JSON，逐則對應回原始項目。
"""
...
SYSTEM_PROMPT = """你是專業的科技新聞編譯，把英文科技新聞翻譯成台灣使用的繁體中文。
...
4. 翻譯要自然通順，像台灣科技媒體的寫法，不要逐字直譯。
5. 只回傳 JSON，不要加任何說明文字或 markdown 標記。"""

USER_PROMPT_TEMPLATE = """請翻譯以下 {count} 則新聞的標題（title）和摘要（summary）。

輸入：
{payload}

回傳格式（JSON object，translations 陣列的每個元素對應一則新聞，id 必須與輸入一致）：
{{"translations": [{{"id": 0, "title_zh": "...", "summary_zh": "..."}}]}}

摘要為空字串時，summary_zh 也回傳空字串。"""


def _build_messages(batch: list[dict]) -> list[dict]:
    payload = json.dumps(
        [{"id": i, "title": it["title"], "summary": it["summary"]} for i, it in enumerate(batch)],
        ensure_ascii=False,
        indent=2,
    )
    return [
...
def _parse_response(text: str, batch_size: int) -> dict[int, dict]:
    """把模型回覆解析成 {id: {title_zh, summary_zh}}，解析失敗回傳空 dict。"""
    text = text.strip()
    ...
    for t in translations:
        ...
        if isinstance(idx, int) and 0 <= idx < batch_size:
            result[idx] = {
                ...
                "summary_zh": str(t.get("summary_zh", "")).strip(),
            }
    return result
...
def translate_items(
    ...
    """批次翻譯。回傳新的 dict 清單，每則多出 title_zh 和 summary_zh 兩個欄位。
    ...
    for bi, batch in enumerate(batches):
        ...
        for i, it in enumerate(batch):
            t = translated.get(i, {})
            out = dict(it)
            out["title_zh"] = t.get("title_zh", f"（翻譯失敗：{last_err}）" if not t else "")
            out["summary_zh"] = t.get("summary_zh", "")
            results.append(out)
